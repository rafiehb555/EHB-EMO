...
class SecurityManager:
    ...
    def record_failed_attempt(self, ip_address: str, username: str):
        ...
        if len(self.failed_attempts[key]) >= self.max_login_attempts:
            ...
            self.log_security_event("BLOCK", f"IP {ip_address} blocked due to multiple failed attempts")
    ...
    def create_session(self, user_id: int, username: str) -> str:
        ...
        self.log_security_event("LOGIN", f"User {username} logged in successfully")
        return token
    
    def validate_session(self, token: str, ip_address: str = None) -> Optional[Dict]:
        ...
        if session["ip_address"] and ip_address and session["ip_address"] != ip_address:
            self.log_security_event("SUSPICIOUS", f"Session IP mismatch for user {session['username']}")
            return None
