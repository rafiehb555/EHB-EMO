...
class GlobalToolsSetup:
    ...
    def setup_global_environment_variables(self):
        ...
        env_vars = {
            "EHB_GLOBAL_TOOLS": str(self.global_tools_dir),
            "EHB_GLOBAL_CONFIG": str(self.global_config_dir),
            "EHB_GLOBAL_CACHE": str(self.global_cache_dir),
            "PATH": f"{self.global_tools_dir / 'scripts'};{os.environ.get('PATH', '')}",
            "PYTHONPATH": f"{self.global_tools_dir / 'python_packages'};{os.environ.get('PYTHONPATH', '')}",
            "NODE_PATH": str(self.global_tools_dir / "node_modules"),
            "NPM_CONFIG_CACHE": str(self.global_cache_dir / "npm"),
            "PIP_CACHE_DIR": str(self.global_cache_dir / "pip")
        }
...
"""
    ...
    def install_global_python_packages(self):
        ...
        python_packages = [
            ...
            "xgboost", "catboost", "lightgbm", "transformers", "openai", "anthropic",
            "langchain", "chromadb", "pinecone-client", "weaviate-client", "qdrant-client",
            
            # Web Development
            "fastapi", "uvicorn", "flask", "django", "streamlit", "gradio", "dash",
            "plotly", "bokeh", "sqlalchemy", "redis", "celery", "pydantic",
            ...
            # Utilities
            "httpx", "aiofiles", "python-multipart", "python-dotenv", "loguru",
            "rich", "psutil", "tqdm", "click", "typer", "marshmallow", "cerberus",
            "jsonschema", "pyyaml", "toml", "configparser", "python-decouple"
        ]
        ...
        for package in python_packages:
            try:
                subprocess.run([
                    ...
                    str(self.global_tools_dir / "python_packages"), package
    ...
    def install_global_npm_packages(self):
        ...
        npm_packages = [
            ...
            "@mui/lab", "react", "react-dom", "react-beautiful-dnd", "@dnd-kit/core",
            "@dnd-kit/sortable", "react-hook-form", "@hookform/resolvers", "yup",
            
            # Routing & State
            "react-router-dom", "redux", "@reduxjs/toolkit", "zustand", "recoil",
            
            # Development Tools
            "typescript", "eslint", "prettier", "husky", "lint-staged", "jest",
            "@testing-library/react", "cypress", "webpack", "vite", "parcel",
            
            # Utilities
            "lodash", "date-fns", "axios", "recharts", "d3", "chart.js",
...
"""
    ...
    def create_global_auto_script(self):
        ...
        auto_script = f'''#!/usr/bin/env python3
"""
...
def setup_global_environment():
    ...
    if (GLOBAL_CONFIG / "cursor" / "global-settings.json").exists():
        with open(GLOBAL_CONFIG / "cursor" / "global-settings.json") as f:
...
"""
    ...
    def create_global_error_fixer(self):
        ...
        error_fixer = f"""#!/usr/bin/env python3
"""
...
"""
        ...
        with open(self.global_tools_dir / "scripts" / "global_error_fixer.py", "w") as f:
            f.write(error_fixer)
