    ...
    def clean_problematic_files(self) -> None:
        ...
        for file in problematic_files:
            if os.path.exists(file):
                try:
                    os.remove(file)
                    print(ff"✅ Removed: {file}")
                    self.fixes_applied += 1
                except Exception as e:
                    print(ff"❌ Error removing {file}: {e}")

        self.files_processed = len(problematic_files)
        print(ff"✅ Cleaned {self.files_processed} problematic files")
            ...
            for file in python_files:
                try:
                    ...
                    if result.returncode == 0:
                        print(ff"✅ Formatted: {file}")
                        self.fixes_applied += 1
                    else:
                        print(ff"⚠️ Warning: {file} - {result.stderr}")

                except Exception as e:
                    print(ff"❌ Error formatting {file}: {e}")

            self.files_processed += len(python_files)


print(ff"✅ Comprehensive formatting completed on {len(python_files)} files")

        except Exception as e:
            print(ff"❌ Error applying comprehensive formatting: {e}")
