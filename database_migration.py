...
class DatabaseMigration:
    ...
    def backup_sqlite_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            source_conn = sqlite3.connect(self.migration_config["source"]["path"])
            ...
            source_cursor.execute("SELECT COUNT(*) FROM sqlite_master WHERE type='table'")
            ...
            if table_count == source_table_count:
                ...
                return {"status": "error", "message": "Backup verification failed"}
    ...
    def create_postgresql_schema(self, schema_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.migration_config["migration_options"]["create_indexes"]:
                self._create_postgresql_indexes(cursor, schema_analysis["schema"]["tables"])
    ...
    def migrate_data(self, schema_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            sqlite_conn = sqlite3.connect(self.migration_config["source"]["path"])
            ...
            print(f"📊 Total records migrated: {migration_stats['total_records_migrated']}")
            
            if migration_stats["errors"]:
                print(f"⚠️ Errors encountered: {len(migration_stats['errors'])}")
            ...
            for table_name, table_info in schema_analysis["schema"]["tables"].items():
                try:
                    ...
                    if sqlite_count == pg_count:
                        ...
                        print(f"❌ {table_name}: {sqlite_count} vs {pg_count} records")
...
- **Tables Migrated**: {migration_data.get('stats', {}).get('tables_migrated', 0)}
- **Total Records**: {migration_data.get('stats', {}).get('total_records_migrated', 0)}
- **Errors**: {len(migration_data.get('stats', {}).get('errors', []))}

## Validation Results
- **Tables Validated**: {migration_data.get('validation', {}).get('tables_validated', 0)}
- **Matching Records**: {migration_data.get('validation', {}).get('matching_records', 0)}
- **Overall Success**: {'✅ Yes' if migration_data.get('validation', {}).get('overall_success', False) else '❌ No'}
...
"""
    ...
    def run_full_migration(self) -> Dict[str, Any]:
        ...
        schema_creation_result = self.create_postgresql_schema(schema_result["schema"])
        if schema_creation_result["status"] != "success":
            return {"status": "error", "message": "PostgreSQL schema creation failed"}
        ...
        if validation_result["status"] != "success":
            return {"status": "error", "message": "Migration validation failed"}
        ...
        print(f"📊 Records migrated: {migration_result.get('stats', {}).get('total_records_migrated', 0)}")
        print(f"✅ Validation: {'Passed' if validation_result.get('validation', {}).get('overall_success', False) else 'Failed'}")
