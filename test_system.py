...
def test_database():
    ...
    try:
        ...
        # Test user creation
        test_user = db.create_user("testuser", "test@example.com", "password_hash", "user")
        print(f"✅ User creation: {'Success' if test_user else 'User already exists'}")
...
def test_auth_manager():
    ...
    try:
        ...
        print(f"✅ Password hashing: {'Consistent' if hash1 == hash2 else 'Inconsistent'}")
        
        # Test password validation
        valid_password = auth.validate_password("strongpass123")
        invalid_password = auth.validate_password("weak")
        print(f"✅ Password validation: {'Working' if valid_password and not invalid_password else 'Failed'}")
...
def test_ai_agents():
    ...
    try:
        ...
        if config_agent:
            result = config_agent.validate_config({"project": "test", "version": "1.0"})
        ...
        if file_agent:
            ...
            print(f"✅ File organizer agent: {result.get('total_files', 0)} files scanned")
        
        # Test code analyzer agent
        code_agent = agent_manager.get_agent("codeAnalyzer")
        if code_agent:
            result = code_agent.analyze_code_quality("main.py")
            print(f"✅ Code analyzer agent: {result.get('quality_score', 'unknown')}")
        ...
        if deploy_agent:
            ...
            print(f"✅ Deployment manager agent: {result.get('ready_for_deployment', False)}")
...
def test_file_system():
    ...
    try:
        ...
        required_files = [
            ...
            "data_processor.py", "ai_agents.py", "config.json", "requirements.txt"
        ]
        ...
        if os.path.exists("config.json"):
            with open("config.json", "r") as f:
                config = json.load(f)
            print(f"✅ Config loaded: {config.get('project', 'Unknown')} v{config.get('version', 'Unknown')}")
        
        # Check requirements.txt
        if os.path.exists("requirements.txt"):
            with open("requirements.txt", "r") as f:
                requirements = f.read()
            print(f"✅ Requirements file: {len(requirements.splitlines())} dependencies")
